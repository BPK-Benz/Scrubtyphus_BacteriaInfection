...
def parse_args():
    ...
    parser.add_argument('--save_path', help='Path to save inference time results')
    args = parser.parse_args()
...
def measure_inference_speed(cfg, checkpoint, max_iter, log_interval,
    ...
    # Create the directory specified by --save_path if it doesn't exist
    if inftime_save_path:
        os.makedirs(inftime_save_path, exist_ok=True)
    ...
    # # Write to the "inference_time.txt" file
    # with open(os.path.join(inftime_save_path, 'inference_time.txt'), 'w') as f:
    #     f.write(f'Overall fps: {fps:.1f} img / s, times per image: {1000 / fps:.1f} ms / img')
